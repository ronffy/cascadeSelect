...
class CascadeSelect extends React.Component {
  ...
  onChange(index, title, e){
    ...
    let list = lodash.cloneDeep(this.state.list);
    let _list = lodash.cloneDeep(this.state._list);
    ...
    return (
      <div style={{display: 'inline-block'}} >
      {
        list.map((item, i) => {
          ...
          return <SearchSelect key={ i } onChange={ ::this.onChange } className="ml-15" { ...o } title="rank" />
        })
      }
      </div>
    );
